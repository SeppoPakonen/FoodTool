...
void WeightCtrl::SelectWeightStat() {
	...
	edit.sentiment.SetIndex(max(0, min<int>(edit.sentiment.GetCount()-1, w.sentiment)));
	edit.health.SetIndex(max(0, min<int>(edit.health.GetCount()-1, w.health)));
	edit.workload.SetIndex(max(0, min<int>(edit.workload.GetCount()-1, w.workload)));
