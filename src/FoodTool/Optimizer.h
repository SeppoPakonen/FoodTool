#ifndef _FoodTool_Optimizer_h_
#define _FoodTool_Optimizer_h_
...
public:
	...
	void MinMax(double min, double max) {min_value = min, max_value = max;}
...
public:
	...
	double GetPopulationEnergy(int i) {return pop_energy[i];}
	const Vector<double>& GetPopulationItem(int i) {return population[i];}
	...
	void SetMaxGenerations(int i) {max_gens = i; max_rounds = max_gens * population_count;}
